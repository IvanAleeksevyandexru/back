...
/** Компонент выбора детских садов */
@Slf4j
@Component
public class KindergartenMapServiceComponent extends AbstractMapComponent<String> implements AttributesDto<MapServiceAttributes> {

    public KindergartenMapServiceComponent(LkNotifierService lkNotifierService,
    ...
    public ComponentType getType() {
        return ComponentType.KindergartenMapService;
    }
    ...
    protected void validateAfterSubmit(Map<String, String> incorrectAnswers, Map.Entry<String, ApplicantAnswer> entry,
        ...
        try {
            NsiDictionary dictionary = getDictionaryForFilter(attributes, scenarioDto, fieldComponent);
            if (NSI_DICTIONARY_EMPTY_PREDICATE.test(dictionary)) {
                if (attributes.useSecondFilter()) {
                    dictionary = getDictionaryForFilter(attributes, scenarioDto, fieldComponent);
                }
                if (NSI_DICTIONARY_EMPTY_PREDICATE.test(dictionary)) {
    ...
    private NsiDictionary getDictionaryForFilter(MapServiceAttributes mapServiceAttributes,
                                                 ...
                                                 FieldComponent fieldComponent) throws JSONException {
        ...
        NsiDictionaryFilterRequest requestBody =
                nsiDictionaryFilterHelper.buildNsiDictionaryFilterRequest(scenarioDto, fieldComponent, presetProperties);
