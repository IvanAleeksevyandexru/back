...
public class RateLimitAnalyticProducerImpl implements RateLimitAnalyticProducer {
    ...
    private final RateLimitAnalyticProducerProps props;

    @Override
    public void send(RateLimitOverHeadDto rateLimitOverHeadDto) {
        try {
            rateLimitOverHeadDtoKafkaTemplate.send(props.getTopicName(), rateLimitOverHeadDto.getUserId(),rateLimitOverHeadDto).get();
