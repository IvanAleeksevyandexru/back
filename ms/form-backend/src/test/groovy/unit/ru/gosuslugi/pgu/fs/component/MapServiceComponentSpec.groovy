...
import ru.gosuslugi.pgu.fs.component.dictionary.MapServiceComponent
...
class MapServiceComponentSpec extends Specification {
    ...
    def mapServiceComponent = new MapServiceComponent(
            lkNotifierService,
    ...
    def 'Map service helper test'() {
        given:
        def fieldComponent = new FieldComponent(type: ComponentType.MapService)
        def scenarioDto = new ScenarioDto()
        ComponentTestUtil.setAbstractComponentServices(mapServiceComponent)

        expect:
        mapServiceComponent.getInitialValue(fieldComponent) != null
        mapServiceComponent.getInitialValue(fieldComponent, scenarioDto) != null
        mapServiceComponent.getType() == ComponentType.MapService
    }
    ...
    def 'Preset value check #1 (No attrs in component description)'() {
        ...
        when:
        mapServiceComponent.process(fieldComponent, scenarioDto, serviceDescriptor)
    ...
    def 'Check validation #1 (Validation OFF)'() {
        ...
        def result = mapServiceComponent.validate(initCurrentValue(), scenarioDto, fieldComponent)
    ...
    def 'Check validation #2 (Error of filter attribute - valueType not found)'() {
        ...
        scenarioDto.applicantAnswers = [address: new ApplicantAnswer(value: 'г. Москва')]
        mapServiceComponent.validate(initCurrentValue(), scenarioDto, fieldComponent)
